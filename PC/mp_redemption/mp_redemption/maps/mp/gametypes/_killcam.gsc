...
waitFinalKillcamSlowdown( startTime )
{
    ...
    if(level.DefaultKillcam == 0)
    {
        ...
        SetTimeScale( 0.3, int( deathTime - 500 ) );
    ...
    {
        secondsUntilDeath = ( ( level.lastKillCam.deathTime - startTime - 75 ) / 1000 );
        deathTime = getTime() + secondsUntilDeath * 1000;
        waitBeforeDeath = 1.7;
        self clientNotify("fkcb");
        wait( max(0, (secondsUntilDeath - waitBeforeDeath) ) );
        SetTimeScale( 0.175, int( deathTime - 400 ) );
        wait( waitBeforeDeath + 1 );
    ...
    {
        secondsUntilDeath = ( ( level.lastKillCam.deathTime - startTime - 1000 ) / 1000 );
        deathTime = getTime() + secondsUntilDeath * 1000;
        waitBeforeDeath = 2.5;
        self clientNotify("fkcb");
        wait( max(0, (secondsUntilDeath - waitBeforeDeath) ) );
        SetTimeScale( 0.15, int( deathTime - 200 ) );
        wait( waitBeforeDeath + 1 );
        SetTimeScale( 1.0, getTime() + 500 );
        wait(.5);
        self clientNotify("fkce");
